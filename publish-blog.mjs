import { saveBlog } from './src/lib/supabase';

// Blog post data matching the enhanced markdown file
const blogData = {
  ...
  content: `# 🎯 Building a Dynamic, Print-Ready Resume Section in React: Moving Beyond Static PDFs

> **"Your resume shouldn't be a static document—it should be a living part of your portfolio that showcases your skills just like your projects do."**
...
> ⚠️ **Key Limitations of Static PDF Resumes:**
...
|----------|---------|----------------|
| **Uses semantic HTML** | Accessibility & SEO | Proper heading structure, lists, sections |
| **Leverages React's component model** | Maintainability | Reusable, composable components |
| **Implements specialized print CSS** | Pixel-perfect output | \`@media print\` stylesheet |
| **Stores data structurally** | Easy updates | Supabase-backed content |
...
💡 **Pro Tip**: This approach (visible in [src/lib/supabase.ts](https://github.com/vedprakash007/react-portfolio/blob/main/src/lib/supabase.ts)) means updating your resume is as simple as editing database entries—no code changes needed.

### ♻️ Reusable Experience Component
...
✨ **Visual Benefit**: The flex-wrap ensures skills flow nicely on different screen sizes while maintaining consistent spacing.

### 🖨️ Print Optimization: Making Browser Match PDF
...
**Result**: A dense but scannable single-page resume that maintains readability.
...
| Metric | Static PDF | Dynamic Web Resume |
|--------|------------|-------------------|
| **Initial load** | ~500KB+ PDF | ~40KB JSON data |
| **Updates** | Regenerate/replace file | Instant database edits |
| **Bandwidth** | Entire file transfer | Only content changes |
| **SEO** | Not indexable | Fully indexable & contributes to page relevance |
...
- [src/components/resume/](https://github.com/vedprakash007/react-portfolio/tree/main/src/components) (Resume components)
- [src/lib/supabase.ts](https://github.com/vedprakash007/react-portfolio/blob/main/src/lib/supabase.ts) (Data layer)
- [src/index.css](https://github.com/vedprakash007/react-portfolio/blob/main/src/index.css) (Print stylesheet)
